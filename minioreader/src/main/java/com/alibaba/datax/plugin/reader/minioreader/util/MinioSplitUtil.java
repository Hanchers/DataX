package com.alibaba.datax.plugin.reader.minioreader.util;

import com.alibaba.datax.common.exception.DataXException;
import com.alibaba.datax.common.util.Configuration;
import com.alibaba.datax.plugin.reader.minioreader.MinioInputStream;
 ...
 */
public class MinioSplitUtil extends UnstructuredSplitUtil {

    private static final Logger LOG = LoggerFactory.getLogger(MinioSplitUtil.class);
    ...
    public MinioSplitUtil(MinioClient minioClient, String bucketName) {
        super(false);
    ...
    public Long getFileTotalLength(String filePath) {
        // 获取object字节总数
        ListObjectsArgs getObjectRequest = ListObjectsArgs.builder()
                .bucket(this.bucketName)
                .prefix(filePath)
                .maxKeys(1)
                .build();
        for (Result<Item> listObject : minioClient.listObjects(getObjectRequest)) {
            try {
                if (listObject.get() == null) {
                    return 0L;
                } else {
                    return listObject.get().size();
                }
            } catch (Exception e) {
    ...
    public InputStream getFileInputStream(StartEndPair startEndPair) {
        return new MinioInputStream(this.minioClient, this.bucketName, startEndPair.getFilePath(),
    ...
    private void reBalanceGroup(List<Group> groups) {
        ...
        FileBlock splitOutBlock = groupMaxLoad.split(splitLen, this.minioClient, this.bucketName);
    ...
    public List<Configuration> getSplitedConfigurations(Configuration originConfiguration, List<Pair<String, Long>> objKeySizePares,
        ...
        this.balanceThreshold = originConfiguration.getDouble(com.alibaba.datax.plugin.reader.minioreader.Key.BALANCE_THRESHOLD, 10.0);
...
class Group {
     ...
     * */
    FileBlock split(Long splitLen, MinioClient minioClient, String ossBucketName) {
        ...
        if (bigBlock.getSize() <= MinioSplitUtil.SINGLE_FILE_SPLIT_THRESHOLD_IN_SIZE) {
            return null;
        }
        ...
        if (splitLen >= bigBlock.getSize()) {
            ...
            InputStream inputStream = new MinioInputStream(minioClient, ossBucketName, outBlock.getObjName(), outBlock.getEndOffset(), originalEnd);
...
class FileBlock {
    ...
    FileBlock(String objName, Long startOffset, Long endOffset) {
        ...
        // 某些情况，endOffset = (-1)-1
        assert (endOffset == -1 || endOffset == -2 || startOffset <= endOffset);
